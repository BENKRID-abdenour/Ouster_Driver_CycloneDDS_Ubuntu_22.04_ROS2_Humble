...
> 💡 **Looking for the Ubuntu 24.04 + ROS 2 Jazzy version?** See the companion repository: [Ouster_Driver_CycloneDDS_Ubuntu_24.04_ROS2_Jazzy](https://github.com/BENKRID-abdenour/Ouster_Driver_CycloneDDS_Ubuntu_24.04_ROS2_Jazzy).
...
3. [Step 1 — Verify ROS 2 Humble Installation](#step-1--verify-ros-2-humble-installation)
...
- **ROS 2 Humble Hawksbill** (LTS, supported until May 2027)
- **`ros-humble-ouster-ros`**
- **`ros-humble-rmw-cyclonedds-cpp`**
- **`ros-humble-cyclonedds`**
- **Linux kernel** 5.15+ or 6.x
...
If ROS 2 Humble is not installed yet, follow the [official installation guide](https://docs.ros.org/en/humble/Installation/Ubuntu-Install-Debs.html) before continuing.
...
- A line starting with `ii` showing the installed version
- A directory listing containing the standard Ouster launch files:
...
- Both packages installed (lines beginning with `ii`)
...
> 💡 **Humble-specific note.** Older builds of `ros-humble-cyclonedds` may use an earlier schema revision, but the XML above is forward-compatible. If CycloneDDS rejects the file on parse, simply remove the `presence_required` attribute and rely on always-up interfaces.
...
Make a note of whether `source /opt/ros/humble/setup.bash` is already present — it must appear **only once** in the file.
...
Append the following block at the very end of the file. **If `source /opt/ros/humble/setup.bash` is already present higher up in the file, do not duplicate it** — leave the `source` line out of the block below.
...
> 💡 **In ROS 2, a bag is a directory**, not a single `.bag` file as in ROS 1. The directory contains `metadata.yaml` plus one or more `.db3` (SQLite) files.
...
A common scenario in research labs is to mix machines running different ROS 2 distributions — for example a Humble workstation talking to a Jazzy robot PC. ROS 2 is generally backward-compatible for standard message types, **provided**:

- Both sides use the **same RMW** (`rmw_cyclonedds_cpp` on both).
- Both sides use the **same `ROS_DOMAIN_ID`**.
- Both sides exchange only **standard message types** (`std_msgs`, `sensor_msgs`, `geometry_msgs`, etc.). Custom message definitions must be compiled on each distribution separately.
- Rosbags recorded on one side use **MCAP** (portable across distributions) rather than the legacy SQLite storage.
